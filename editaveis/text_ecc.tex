...
Seja a curva elíptica $E$ definida sobre o corpo finito $\mathbb{F}_q$. O conjunto de todos os pontos que pertencem à curva elíptica é representado por $E(\mathbb{F}_q)$ (ver \ref{eq:pontosCurva}). A quantidade de pontos que compõe esse conjunto é a \textbf{ordem de $E$ sobre $\mathbb{F}_q$} e será representada pela notação $\#E(\mathbb{F}_q)$. \cite{Guide}
