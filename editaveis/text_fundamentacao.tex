...
Em matemática, relação é o estudo de como os elementos de um conjunto $E$, ou de dois conjuntos distintos(relação binária), se relacionam entre si\cite{Domingues:2003}. O conceito de relação pode ser compreendido de maneira mais fácil partindo-se da definição de outro conceito, o de produto cartesiano.
DEFINIÇÃO: Sejam dois conjuntos não vazios \(E\) e \(F\), o \textit{produto cartesiano} entre esses dois conjuntos, representado por \(E \times F\), é o conjunto de todos os pares ordenados \((x, y)\), tais que \(x \in E\) e \(y \in F\).

\begin{equation}
  E \times F = {(x, y) | x \in E \mbox{ e } y \in F}
\end{equation}

Assim, defini-se \textit{relação binária} de \(E\) em \(F\) como qualquer subconjunto de \(E \times F\), ou seja, uma relação qualquer que une um \(x \in E\) a um \(y \in F\)\cite{Domingues:2003}. Quando definida sobre um conjunto \(E\), algumas propriedades da relação são:
...
Uma relação que satisfaz as propriedades reflexiva, simétrica e transitiva citadas acima, é chamada de \textit{relação de equivalência}\cite{Domingues:2003}.
