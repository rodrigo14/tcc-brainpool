...
Seja um grupo \(G\), um subgrupo \(H\) de \(G\), e um elemento \(a \in G\), chama-se \textbf{classe lateral à direita}, módulo \(H\), determinada por \(a\), qualquer subconjunto de \(G\) formado pelos elementos \(Ha\) tais que \(Ha = \{ha\ |\  h \in H\}\) \cite{Gilbert:2004}. Por exemplo, considere um grupo multiplicativo formado pelo conjunto \(\mathbb{Z}_7^* = \{1, 2, 3, 4, 5, 6\}\), agora considere o subconjunto \(H = \{1, 4, 5\}\). As classes laterais à direita são dadas por
\begin{gather*}
  H\cdot1 = \{1\ \cdot\ 1,\ 4\ \cdot\ 1,\ 5\ \cdot\ 1\} = \{1, 4, 5\} \\
  H\cdot2 = \{1\ \cdot\ 2,\ 4\ \cdot\ 2,\ 5\ \cdot\ 2\} = \{2, 1, 3\} \\
  H\cdot3 = \{1\ \cdot\ 3,\ 4\ \cdot\ 3,\ 5\ \cdot\ 3\} = \{3, 5, 1\} \\
  H\cdot4 = \{1\ \cdot\ 4,\ 4\ \cdot\ 4,\ 5\ \cdot\ 4\} = \{4, 2, 6\} \\
  H\cdot5 = \{1\ \cdot\ 5,\ 4\ \cdot\ 5,\ 5\ \cdot\ 5\} = \{5, 6, 4\} \\
  H\cdot6 = \{1\ \cdot\ 6,\ 4\ \cdot\ 6,\ 5\ \cdot\ 6\} = \{6, 3, 2\}
\end{gather*}
De forma análoga, as \textbf{classes laterais à esquerda} são definidas pelos subconjuntos \(aH = \{ah\ |\ h \in H\}\).
Pode-se perceber que todas as classes laterais tem a mesma cardinalidade, ou seja, possuem a mesma quantidade de elementos.

%
% DEMONSTRAÇAO DO TEOREMA DE LAGRANGE
%
