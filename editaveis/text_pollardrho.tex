...
\begin{equation}
\dfrac{1}{M} \sqrt{\dfrac{\pi n}{2}} + \dfrac{1}{\theta} \label{eq:execParalized}
\end{equation}
...
A função $g(R)$ é utilizada como função de iteração para o algoritmo Pollard rho paralelizado. Essa modificação garante ao algoritmo uma melhoria no tempo de execução do Pollard rho paralelizado, dessa forma, a equação \ref{eq:execParalized} passa a ser
$$
